...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Defaults to models/checkpoints/<artifact-prefix>_best.pt.",
    )
    parser.add_argument(
        "--reports-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Prefix used for all generated report file names.",
    )
...
def main() -> None:
    ...
    checkpoint_path = args.checkpoint or (
        PROJECT_ROOT / "models" / "checkpoints" / f"{args.artifact_prefix}_best.pt"
    )
    ...
    device = get_device()
    model, _ = load_cnn_checkpoint(checkpoint_path, device=device)
    ...
    slice_pred_path = args.reports_dir / f"{prefix}_test_slice_predictions.csv"
    group_pred_path = args.reports_dir / f"{prefix}_test_group_predictions.csv"
    metrics_path = args.reports_dir / f"{prefix}_test_metrics.csv"
    comparison_path = (
        args.reports_dir / f"{prefix}_vs_statistical_baseline_test.csv"
    )
